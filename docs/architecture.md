...
| **M23** | **Loot caches — "things in the fog" (unowned `Cache` structures, owner sentinel `-2`, scattered across the world AT GENESIS only on tiles NO player has ever seen — the fog gate reads every player's starting-vision discs; deterministic sim-`Rng` scatter + loot roll in the spec-ctor, `CacheConfig.Count` defaults 0 → zero perturbation of existing scenarios; a cache is a `StorageStructure` so it round-trips on the existing payload, and the `-2` owner makes "vanishes back into the fog when unseen" fall out of `BuildPlayerView` for free; looted by `LootCacheIntent` cargo-capped from the M16 cargo atom, remainder persists, removed when empty, contents resources+gear; `ViewProjector` reveals a VISIBLE cache's loot; `FormatVersion` 16→17, zero new anchors → recovery-clean; complements M22 — visible peaks to race, hidden caches to discover. Respawns/guarded caches deferred. See `docs/loot-caches.md`)** | **✅** |
| **M22** | **High-terrain visibility (the highest band — `Biome.Mountain`, the scarcest resources — is common knowledge: revealed to every player's `RememberedTerrain` from the start so the opening is a race to the peaks, not a fog-gated discovery. TERRAIN ONLY — units/structures/roads on a mountain stay fogged until real vision; revealed via the pure-read `View.BuildPlayerView`, never `Explored` (so the road overlay can't leak), so both the client and the AI brain see it through the same fair channel. `Biomes.IsCommonKnowledgeTerrain` is the single knob; `GameWorld.CommonKnowledgeTerrain` is an immutable, non-serialized, hash-invisible memo computed once from the frozen grid → zero sim-state/snapshot change, M3 fog headline intact. See `docs/high-terrain-visibility.md`)** | **✅** |
...
| **M24** | **Sieges & conquest — the win condition shipped (`Structure.Health` on the base class, seeded by `StructureCatalog.BaseHealth`, sentinel 0 = indestructible; Castle 1000 / Barracks 200 / Tower 150 / Stockpile 100 / extractor + House + Lodge + School + Dock 50–75 / ConstructionSite 25 / Cache + Canal + Rubble 0; new `StructureKind.Rubble` owner sentinel `-3`, persists forever, blocks placement — `Structures.ContainsKey` already rejects, zero intent code); `CombatRoundEvent.Apply` folds siege damage after unit damage, defenders shield at ROUND START (death this round still costs a round), damage = Σ attackers' `EffectivePower`, no new balance knob; combat-start broadens to lone attacker + hostile destructible structure (the empty-castle gate); razing a Castle schedules a `PlayerDefeatedEvent` → `Player.Defeated` → `GameOverEvent` if ≤1 non-bandit/non-cache player remains; defeated-player intents reject at the `IntentEvent` wrapper (single gate, every intent class muted); bandits exempt from siege (M16 deferral honored); `FormatVersion` 18→19 (`Structure.Health` + `Player.Defeated`), zero new anchors → both transitions are same-tick scheduled events; headline = twin-run hash equality across siege → game-over + mid-siege snapshot → restore → finish-siege hash equality. Repair / rubble decay / castle capture deferred. See `docs/sieges-and-conquest.md`)** | **✅** |
| **M25** | **The Rival — war-capable AI (builds on the M17 Homesteader). Per-AI PERSONALITY (`AiPersonality` Homesteader/Opportunist/Warlord) seeded deterministically from `(worldSeed, ownerId)` on the ephemeral `AiConfig` — Homesteader default = today's brain, zero perturbation. New `RivalRung`: casus belli (encroachment / retaliation / opportunism / land hunger) + the telegraphed `DeclareWarIntent` run in a PERCEPTION pass every think (timely, never starved by the economy); the army DIRECTION is a low ladder rung below Muster (economy-first, never marches a starving colony). Strike doctrine military → economy → Castle; siege reuses M24 on contact; war termination = Warlord presses to conquest, Opportunist sues for peace (M6 bilateral handshake), any non-Warlord accepts. Public diplomacy reaches the brain through the SAME fair `ViewDto` a human renders (new `Factions`/`Relationships`/`PendingWars`/`IncomingProposals`, pure-read projection); enemy power estimated from visible ROLE (true power stays fogged). Fourth instance of the M16 driver shape — zero new sim mutation points, zero anchors, no snapshot change; headline = AI-driven war deterministic across twin-run + replay-from-intent-log. Distant-enemy hunting / group field armies / naval invasion / multi-front deferred. See `docs/ai-rival.md` + `docs/m25-rival-spec.md`)** | **✅** |
