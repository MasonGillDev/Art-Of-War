...
public static class MovementConstants
{
    ...
    //   1-3 units  : +0           (normal play — solo units, small chains)
    //   4-7 units  : +SmallBand   (small group, noticeable cost)
    //   8-15 units : +MediumBand  (medium army, sluggish)
    //   16+ units  : +LargeBand   (massive pileup, very slow)
    ...
    //
    // Band values are pegged at +1/+2.5/+5 grassland-tiles-worth so the
    // deterrent scales WITH the movement tempo: a 16+ crowd must cost more
    // than a short detour around it, whatever the terrain numbers are.
    // (At the original grassland cost 10 these were 10/25/50; the ×3
    // movement retune moved them in lockstep.)
    public const int SmallBand  = 30;
    public const int MediumBand = 75;
    public const int LargeBand  = 150;

    public static int BandedCrowdingCost(int unitCount)
    {
        if (unitCount <= 3)  return 0;
        if (unitCount <= 7)  return SmallBand;
        if (unitCount <= 15) return MediumBand;
        return LargeBand;
    }
}
